...
<tbody>
	...
	$qry = $isAdmin ? $conn->prepare("SELECT * FROM `invoice_list` WHERE doc_type = 'order' ORDER BY DATE(date_created) DESC") : $conn->prepare("SELECT * FROM `invoice_list` WHERE created_by = $id AND doc_type = 'order' ORDER BY DATE(date_created) DESC");    // $qry->bind_param("s", $loggedInUserId);
    ...
    while($row = $result->fetch_assoc()):
        ...
        // $items = $conn->query("SELECT * FROM order_items WHERE order_id = {$row['id']}")->num_rows;
    ?>
        <tr>
            ...
            <td align="center">
                ...
                <div class="dropdown-menu" role="menu">
                    ...
                    <!-- <a class="dropdown-item delete_data" href="javascript:void(0)" data-id="<?php echo md5($row['id']) ?>"><span class="fa fa-trash text-danger"></span> Delete</a> -->
                </div>
            </td>
			<?php endif ?>
