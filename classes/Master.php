...
Class Master extends DBConnection {
	...
	function save_expense(){
		...
		$data = "";
		foreach($_POST as $k =>$v){
			if(!in_array($k,array('id','category'))){
				if(!empty($data)) $data .=",";
				if($k == 'price')
				$v = floatval(str_replace(',','',$v));
				$data .= " `{$k}`='{$v}' ";
		...
		if(isset($_POST['category'])){
			if(!empty($data)) $data .=",";
				$data .= " `category`='".addslashes(htmlentities($category))."' ";
		}
	...
	function generate_code(){
		...
		while (true) {
			...
			if ($type === 'order') {
				$code = 'SFPORD-'; // Change code prefix for Order
			}
			else{
				$code = 'SFPINV-'; // Default code prefix for Invoice
			}
			$code .= sprintf("%'.07d", mt_rand(1, 9999));
