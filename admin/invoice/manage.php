...
// $doc_type = isset($_SESSION['system_info']['doc_type']) ? $_SESSION['system_info']['doc_type'] : "order"; 
...
<div class="card card-outline rounded-0 card-maroon">
	...
	<h3 class="card-title"><?php echo !isset($_GET['id']) ? "New Record" :"Edit Record" ?></h3>
	</div>
	<div class="card-body">
				...
				<div class="row">
					<div class="col-md-6">
							<div class="form-group">
								<label for="doc_type" class="control-label">Select Appropiate Doc Type</label>
								<select name="doc_type" id="doc_type" class="custom-select custom-select-sm select" >>
									<option value="invoice" <?php echo isset($doc_type) && $doc_type == 'invoice' ? 'selected' : '' ?>>Invoice</option>
									<option value="order" <?php echo isset($doc_type) && $doc_type == 'order' ? 'selected' : '' ?>>Order</option>
								</select>
							</div>
						</div>
						<div id="orderStatusSection" class="col-md-6" style="display: none;">
							<div class="form-group">
								<label for="status" class="control-label">Select Order Status</label>
								<select name="status" id="status" class="custom-select custom-select-sm select">
				...
				<div class="row">
					...
					<div class="col-md-6">
						<div class="form-group">
							...
							<input type="text" name="invoice_code" value="<?php echo isset($invoice_code) ? $invoice_code : ''; ?>"  class="form-control form-control-sm" required readonly>
						</div>
						<div class="form-group">
					...
					<div class="col-md-6">
						<div class="form-group">
							...
							<select name="type" id="type" class="custom-select custom-select-sm select" >>
					...
					<div class="col-md-12">
						<table class="table table-bordered" id="item-list">
							...
							<tfoot>
								...
								<tr>
									<th class="text-right" colspan="4">Fuel Charges</th>
									<!-- <th class="text-right" id="tax_rate"><?php //echo $cgst_rate ?>%</th> -->
									<th style="text-align: right;"><input type="hidden" name="tax_rate" value="">10</th>
								</tr>

								<tr>
									<th class="text-right" colspan="4">Other Charges</th>
									<!-- <th class="text-right" id="tax_rate"><?php //echo $cgst_rate ?>%</th> -->
									<th style="text-align: right;"><input type="hidden" name="tax_rate" value="">10</th>
								</tr>
								<!-- <tr>
		...
		<button class="btn btn-flat btn-sm btn-default bg-maroon" form="invoice-form" onclick="cal_pending()">Save</button>
...
function generate_code(){
    start_loader();
    var selectedDocType = $('#doc_type').val(); // Get the selected document type from the dropdown
    $.ajax({
        url: _base_url_ + 'classes/Master.php?f=generate_code',
        method: 'POST',
        data: { type: selectedDocType }, // Send the selected document type
        dataType: "json",
        error: err => {
            console.log(err);
            alert_toast('An error occurred while generating code', 'error');
        },
	...
	function calc_total(){
		...
		var gtotal = fuelCharges + otherCharges; 
...
<script>
    // Function to show/hide the orderStatusSection based on doc_type selection
    function toggleOrderStatusSection() {
        var docTypeSelect = document.getElementById("doc_type");
        var orderStatusSection = document.getElementById("orderStatusSection");

        if (docTypeSelect.value === "order") {
    ...
    // Attach an event listener to the doc_type dropdown to call the toggle function
    document.getElementById("doc_type").addEventListener("change", toggleOrderStatusSection);

    // Initial call to set the initial visibility state based on the initial selection
    toggleOrderStatusSection();
</script>
